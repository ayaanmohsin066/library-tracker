      ...
      <div className="flex flex-col md:flex-row" style={{ minHeight: "500px" }}>
        ...
        <div className="md:order-1" style={{ flex: "0 0 45%", padding: "36px 40px", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 12 }}>
            ...
            <span style={{ fontSize: 13, color: lib.isOpen ? "var(--green)" : "var(--red)", fontWeight: 600 }}>{lib.isOpen ? "Open" : "Closed"}</span>
...
export default function ReginaPage() {
  const { data, isLoading, isError, isStale, dataUpdatedAt, refetch } = useLibraryData("regina");
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "var(--bg-base)" }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", padding: "88px 24px 48px" }}>
        <div aria-hidden="true" style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, background: "radial-gradient(ellipse 70% 60% at 50% 50%, rgba(129,140,248,0.05) 0%, transparent 70%)", pointerEvents: "none" }} />
        <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative", zIndex: 1 }}>
          ...
          <p style={{ fontSize: 15, color: "var(--text-secondary)", display: "flex", alignItems: "center", flexWrap: "wrap", gap: 6 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 7 }}>
              <span className="animate-pulse" style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", backgroundColor: "#10b981", flexShrink: 0 }} />
              <span style={{ color: "#10b981", fontWeight: 600 }}>Live</span>
            </span>
            library occupancy — updated every 5 minutes
          </p>
          {dataUpdatedAt && <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>{dataUpdatedAt}</p>}
        </div>
      </section>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 16px 80px" }}>
        ...
        {!isLoading && libraries.length > 0 && (
          <div style={{ marginBottom: 24 }}><BestSpotCard libraries={libraries} /></div>
        )}
        ...
        {!isLoading && libraries.length > 0 && (
          <div style={{ display: "flex", flexDirection: "column" }}>
            {libraries.map((lib, idx) => (
              <div key={lib.name}>
                {idx > 0 && (
                  <div style={{ height: 1, margin: "12px 0", background: "linear-gradient(to right, transparent, rgba(129,140,248,0.22), transparent)" }} />
                )}
                <LibraryRow lib={lib} />
              </div>
            ))}
